...
int* tcp_last_seen_sensor_id(tcpsock_t* socket) {
    return &socket->last_seen_sensor_id;
}

time_t* tcp_last_seen(tcpsock_t* socket) {
    return &socket->last_seen;
}
